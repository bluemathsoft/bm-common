...
var NDArray = /** @class */ (function () {
    ...
    NDArray.prototype.indexToAddress = function () {
        var indices = [];
     ...
     * @hidden
     */
    NDArray.prototype.addressToIndex = function (addr) {
        if (addr >= this._size) {
    ...
    NDArray.prototype.set = function () {
        ...
        if (this.isSliceIndex(index)) {
            ...
            var addr = this.indexToAddress.apply(this, index);
    ...
    NDArray.prototype.forEach = function (callback) {
        for (var i = 0; i < this._size; i++) {
            var index = this.addressToIndex(i);
            if (this._idata[i] === undefined) {
    ...
    NDArray.prototype.get = function () {
        ...
        if (nranges === 0) {
            ...
            var addr = this.indexToAddress.apply(this, idx);
        ...
        for (var i = 0; i < slicesize; i++) {
            // Convert each address of slice array to index
            var newidx = slicearr.addressToIndex(i);
            // Find index into the original array (oldidx) that corresponds
            // to the newidx
            var oldidx = [];
            var rangecount = 0;
        ...
        if (axis !== undefined && axis !== null) {
            if (typeof axis === 'number') {
                ...
                for (var i = 0; i < maxsize; i++) {
                    var maxindex = maxarr.addressToIndex(i);
